...
- **左栏**（292px）：数据操作（读取/重新爬取/智能分类）、筛选条件（搜索/分类/预算/远程/黑名单/学生模式）、排序方式、导出——所有控制集中一处，竖排排列更易阅读
- **中栏**：顶部统计胶囊（总数/显示数/远程/学生友好），中部项目表格（斑马纹/粘性表头/排序指示），底部「项目详情 / 爬取日志」标签页
- 学生模式行绿色高亮、黑名单红标、远程蓝标，一眼可辨
